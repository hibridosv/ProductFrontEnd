...
useEffect(() => {
  if (!isAdContactModal) {
    (async () => setContacts(await loadData(`contacts?sort=-created_at&perPage=10${currentPage}${searchTerm}&filterWhere[status]==1&filterWhere[is_employee]==1`)))();
    setLinks([{"name": `Descargar repartidores` , "link": encodeURI(`${remoteUrl}/download/excel/contacts/?sort=name&filterWhere[is_employee]==1`), "isUrl": true},])
  }
