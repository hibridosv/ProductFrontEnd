...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const handlegetSales = async (data: DateRangeValues) => {
        try {
          setIsSending(true);
          let url = urlConstructor(data, 'histories/shipping-notes')
          const response = await getData(url);
          if (!response.message) {
            toast.success("Datos obtenidos correctamente");
            setSales(response);
            if(response.data.length > 0) addLink(links, data, 'excel/shipping-notes/');
          } else {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-10 pb-10">
        ...
        <HistoriesShippingNoteTable records={sales} isLoading={isSending} />
