...
import { HistoriesDeletedTable } from "@/components/histories-components/histories-deleted-table";
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const handlegetSales = async (data: DateRangeValues) => {
        try {
          setIsSending(true);
          const response = await postData(`histories/deleted`, "POST", data);
          if (!response.message) {
            toast.success("Datos obtenidos correctamente");
            setSales(response);
            if(response.data.length > 0) addLink(links, data, 'excel/deleted/');
          } else {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-10 pb-10">
        ...
        <ViewTitle text="LISTADO DE ORDENES ELIMINADAS" />

        <HistoriesDeletedTable records={sales} isLoading={isSending} />
