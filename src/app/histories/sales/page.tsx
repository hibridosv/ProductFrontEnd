...
export default function Page() {
  ...
  const handlegetSales = async (data: DateRangeValues) => {
    try {
      setIsSending(true);
      let url = urlConstructor(data, 'histories/sales')
      const response = await loadData(url);
      if (!response.message) {
        toast.success("Datos obtenidos correctamente");
        setSales(response);
        if(response.data.length > 0) addLink(links, data, 'excel/sales/');
      } else {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-10 pb-10">
        ...
        <HistoriesSalesTable records={sales} isLoading={isSending} />
