...
export function DeliveryContactList(props: DeliveryContactListProps) {
  ...
  const listItems = records.data.map((record: any) => (
    <div className="w-full bg-teal-50 px-2" key={record.id}>
        ...
        <div className="mx-4 font-bold clickeable flex" title="Ver información" onClick={()=>handleRecordSelectView(record)}><GoEye className="mt-1 mr-2 " color="green" /> { record?.name }</div>
