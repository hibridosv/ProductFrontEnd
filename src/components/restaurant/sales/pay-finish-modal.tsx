        ...
        <div className="mx-4">
              ...
              <div className="w-full my-4">
                ...
                <div  className='flex justify-between  border-y-4'>
                  <div><span className="flex justify-center">Descuentos</span> <span className="flex justify-center">{numberToMoney(invoice?.discount)}</span></div>
                  <div><span className="flex justify-center">Impuestos</span> <span className="flex justify-center">{numberToMoney(invoice?.taxes)}</span></div>
                  <div><span className="flex justify-center">Sub Total</span> <span className="flex justify-center">{numberToMoney(invoice?.subtotal)}</span></div>
                </div>
                }
                { isSending ? <Loading text="Procesando..." /> :
                <div>
                    <div className="flex justify-center mt-4">TOTAL</div>
                    <div className="flex justify-center text-7xl mb-4 font-bold">{numberToMoney(invoice?.total - invoice?.retention)}</div>
                    { invoice?.payment_type === 1 && invoice?.invoice_assigned?.type != 8 ? <>
                    <div className="flex justify-center">CAMBIO</div>
                    <div className="flex justify-center text-7xl mb-4 text-red-600 font-bold">{numberToMoney(invoice?.change)}
                    </div></> : 
