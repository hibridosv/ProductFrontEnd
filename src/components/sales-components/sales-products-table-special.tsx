...
export interface SalesProductsTableSpecialProps {
  order: any
  onClickProduct: (product: Product, option: OptionsClickSales)=>void
}

export function SalesProductsTableSpecial(props: SalesProductsTableSpecialProps) {
  const { order, onClickProduct } = props;
  ...
  const remoteUrl = getUrlFromCookie();
        ...
        const listItems = special && special.map((record: any) => (
            ...
            <td className='py-3 px-6 clickeable'>{ record.quantity }</td>
